...
from .datasets_new import CIFAR10_truncated
import os
...
#change            transforms.ToPILImage(),
    ...
    valid_transform = transforms.Compose(
        [
#change            transforms.ToPILImage(),       # Convert numpy.ndarray to PIL.Image
            transforms.Resize(64), 
            transforms.ToTensor(),
            #transforms.Normalize(CIFAR_MEAN, CIFAR_STD),
            transforms.Normalize(IMAGE_NET_MEAN, IMAGE_NET_STD)
...
def get_dataloader_CIFAR10(datadir, train_bs, test_bs, dataidxs=None, save_preprocessed=True):
    ...
    # Load or preprocess test dataset
    if save_preprocessed and os.path.exists(test_save_path):
        ...
        print("Processing and saving test dataset...")
...
#    test_ds = dl_obj(datadir, train=False, transform=transform_test, download=True)
    ...
    train_generator.manual_seed(42)  # Set seed for reproducibility

    test_generator = torch.Generator()
    test_generator.manual_seed(142)  # Set seed for reproducibility

    train_dl = data.DataLoader(
        dataset=train_ds, batch_size=train_bs, shuffle=True, drop_last=True, generator = train_generator
    )
    test_dl = data.DataLoader(
        dataset=test_ds, batch_size=test_bs, shuffle=False, drop_last=True, generator = test_generator
    )
