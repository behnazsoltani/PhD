...
#    print(f"rank: {rank}, param:{flatten_tensors(tensorlist)}")
   ...
   # print(f"rank: {rank}, local sample ratio: {DataRatios[rank]}") 
    ...
    if args.comm_style == 'ld-sgd':
        communicator = decenCommunicator(rank, size, MPI.COMM_WORLD, GP, args.selection, args.utility_metric, args.i1, args.i2)
...
#                
#                process_local = multiprocessing.Process(target=evaluate_local, args=(queue_local,model,data))
#                process_global = multiprocessing.Process(target=evaluate_global, args=(queue_global,model,model_evaluation,global_params,data))
#                
#                process_local.start()
#                process_global.start()
#                
#                process_local.join()
#                process_global.join()
#                
#                output_local = queue_local.get()
#                output_global = queue_global.get()
                ...
                with torch.no_grad():
                    ...
                    #if rank == 6:
                    #    print("middle time:", time.time() - middle)
                ...
                output_local, _ = model(data)
                    
                loss_local = criterion(output_local, target) 
                    
                #loss = loss_local + 0.05*(output - output_global).norm(2)
                loss = loss_local + args.lambda_kd*(output_local - output_global).norm(2)
...
#                n_batch = 0
            #if rank == 6:
             #   print("local time:", time.time() - begin)
...
#            optimizer.step()
 #           optimizer.zero_grad()
            ...
            if d_comm_time > 0:
               ...
               # print("rank: %d, test_acc: %0.3f"% (rank, test_acc))
...
#    recorder.save_to_file()
    ...
    with open(f'accuracy_results_{args.name}.txt', 'a') as file:
#    with open(f'accuracy_results_{args.layer}.txt', 'a') as file:
        file.write(f"seed: {args.randomSeed}, Rank: {rank},  Accuracy = {test_acc:.2f}\n")
    total_time = time.time() - init_time
    with open(f"time_results.txt", 'a') as file:
        if rank == 5:
          file.write(f"{args.name}:\n")
          file.write(f"Time = {total_time:.2f}\n")
...
def evaluate_global(queue, model, model_evaluation, global_params, data):
    with torch.no_grad():
        #local_params = [p.detach().clone() for p in model.parameters()]
        #model.load_state_dict(global_params.state_dict())
                    #start_kd = time.time()
    ...
    queue.put(output_global)

def evaluate_local(queue, model, data):
    #torch.cuda.init()
    output, _ = model(data)
