...
class GraphConstruct:
    ...
    def __init__(self, rank, size, comm, graph, weight_type, p=0.5, num_c=None, rows=None, columns=None):
        ...
        if self.size > 1:
            ...
            self.neighbor_list = self.getNeighbors(rank, graph)
    ...
    def selectGraph(self, graph, p, num_c):
        ...
        if isinstance(graph, list):
            ...
            if graph == 'fully-connected':
                ...
                #def construct_2d_torus(rows, cols):
                G = nx.grid_2d_graph(self.rows, self.columns, periodic=True)  # Create a 2D torus graph
                #print("Edges for grid:", G.edges())

                # Connect nodes in a torus-like fashion
                #for i in range(rows):
                #for i in range(4):
                #    for j in range(4):
                    # Connect the right neighbor to form a horizontal ring
                        #g.add_edge((i, j), ((i + 1) % rows, j))
               #         G.add_edge((i, j), ((i + 1) % 4, j))

                        # Connect the bottom neighbor to form a vertical ring
                        #g.add_edge((i, j), (i, (j + 1) % cols))
                #        G.add_edge((i, j), (i, (j + 1) % 4))


          
                # Print the nodes and edges of the 2D torus topology
                g = G.edges()
                #print("Edges:", G.edges())
            elif graph == 'grid':
                G = nx.grid_2d_graph(self.rows, self.columns, periodic=False)
                g = G.edges()
                
            elif graph == 'random':
                random_graph = nx.random_regular_graph(np.random.randint(2, 6), self.size)
                g = random_graph.edges
    ...
    def getNeighbors(self, rank, graph):
        
        neighbors = [[] for _ in range(self.size)]
        for edge in self.graph:
            node1, node2 = edge[0], edge[1]
            
            #print(f"rank: {rank}, edge0: {edge[0]}, edg1: {edge[1]}")
            if node1 == node2:
                print("Invalid input graph! Circle! ("+str(node1) +", "+ str(node2)+")")
                exit()
            if graph == 'torus' or graph =='grid':
                #print("hiiiiiiiiiiiiiii ", graph)
                neighbors[node1[0] * self.columns + node1[1]].append(node2[0] * self.columns + node2[1])
                neighbors[node2[0] * self.columns + node2[1]].append(node1[0] * self.columns + node1[1])
            else:
                neighbors[node1].append(node2)
                neighbors[node2].append(node1)
                
        print(f"rank:{rank}, neighbors:{neighbors[rank]}")
