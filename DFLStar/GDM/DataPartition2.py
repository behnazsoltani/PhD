...
class DataPartitioner(object):
    ...
    def getNonIIDdata(self, rank, data, partition_sizes, degree_noniid, val_split=0.25, seed=1234):
        ...
        while rem != 0:
            ...
            if rem > 0:
                rem -= 1
                total_label_niids[rem%num_labels] += 1
            else:
                total_label_niids[rem%num_labels] -= 1
                rem += 1
        ...
        for worker_idx in range(num_workers):            
            ...
            shuffled_bins = list(range(num_labels))
            random.seed(seed)
            random.shuffle(shuffled_bins)

            to_fill = worker_split[worker_idx][0]
            bin_idx = 0  # Initialize the index for shuffled bins


            # Fills up worker with to_fill noniid data
            to_fill = worker_split[worker_idx][0]
            part_fill = int(to_fill/3)
            while to_fill > 0:
                curr_bin = shuffled_bins[bin_idx]
                num_in_bin = len(labelIdxDict[curr_bin])
                # How much to take from the bin/label
                take_num = min(part_fill, to_fill, num_in_bin)
                to_fill -= take_num
...
#                curr_bin %= num_labels
        ...
        #print(f"size for rank {rank} is {total_worker_sizes[rank]}")
...
def partition_dataset(rank, size, comm, val_split, args):
    ...
    if downloadDataset == 1:
        if args.dataset == 'cifar10':
            ...
            torchvision.datasets.utils.download_and_extract_archive(url, datasetRoot, filename=filename)
        elif args.dataset == 'mnist':
            url = "https://storage.googleapis.com/tensorflow/tf-keras-datasets/mnist.npz"
            filename = "mnist.npz"
            # Download the MNIST dataset
            urllib.request.urlretrieve(url, os.path.join(datasetRoot, "mnist.npz"))
        else:
    ...
    if args.dataset == 'cifar10' or args.dataset == 'cifar100':
        ...
        trainset = dataset_class(root=datasetRoot,
                                                train=True,
                                                download=True,
        ...
        print("The number of labels: ", len(trainset.targets))
        ...
        # To get the number of batches in the DataLoader:
        num_batches = len(train_loader)

# To get the total number of samples in the DataLoader, you can sum the batch sizes
        total_samples = len(train_loader.dataset)
        print("Train data Number of batches:", num_batches)
        print("Train data Total number of samples:", total_samples)
        ...
        testset = dataset_class(root=datasetRoot, train=False, download=True,
...
# To get the total number of samples in the DataLoader, you can sum the batch sizes
        ...
        print("Test data Number of batches:", num_batches)
        print("Test data Total number of samples:", total_samples)
        comm.Barrier()
...
#                                              transform=transform_train)
        ...
        trainset = datasets.MNIST(datasetRoot, train=True, download=True, transform=transform_train)
        #testset = datasets.MNIST(data_dir, train=False, download=True, transform=test_transform)
        
        
        
#        dataset = np.load(os.path.join(datasetRoot, "mnist.npz"))
...
#        y_test = torch.from_numpy(y_test).long()
        ...
        testset = datasets.MNIST(datasetRoot, train=False, download=True, transform=transform_test)
