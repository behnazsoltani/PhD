...
from Utils.Misc import AverageMeter, compute_softmaxprob, test_accuracy, Recorder, test_loss, KL_Loss
import copy
import struct
...
class clientSelection:
    ...
    def __init__(self, rank, model, args, size, comm, topology, selection, utility_metric, ratio, i1, i2):
        self.comm = comm
        self.rank = rank
        self.size = size
...
#                        print(f"rank: {self.rank} model parameter after averaging: {self.recv_buffer}")
    ...
    def communicate(self, model, test_loader, loss, current_data_list):
...
#            print(f"rank: {self.rank} parameters: {flatten_tensors(self.tensor_lst).cpu()}")
        ...
        if self.iter % (self.i1+1) == 0:
           ...
           self.reset_model()
            ...
            neighbor_features_dict, neighbor_logits_dict = (
            self.sendreceive_knowledge_neighbor(self.list_features, self.list_outputs))
            
            #print(f"list_features: {self.list_features}, list_ouputs: {self.list_outputs}")
            
            self.train_neighbor_model(model, neighbor_features_dict, neighbor_logits_dict)
...
#                    self.global_parameter = torch.cat((self.global_parameter, param.reshape(-1)), 0)
    ...
    def sendreceive_knowledge_neighbor(self, extracted_feature_dict, logits_dict):
        self.comm.Barrier()
        tic = time.time()
        neighbor_extracted_feature_dict = dict()
        neighbor_logits_dict = dict()
        neighbor_labels_dict = dict()
        
        knowledge = (extracted_feature_dict, logits_dict)
        for node in self.neighbor_list:
            info = self.comm.sendrecv(knowledge, dest=node, source=node)
            (neighbor_feature, neighbor_logits) = info
            neighbor_extracted_feature_dict[node] = neighbor_feature
            neighbor_logits_dict[node] = neighbor_logits
             
        self.comm.Barrier()
        toc = time.time()
        return neighbor_extracted_feature_dict, neighbor_logits_dict
        
    def train_neighbor_model(self, model, neighbor_extracted_feature_dict, neighbor_logits_dict):
        model.train()
        #self.n_iter +=1
#        for name, layer in model.named_modules():
#        #if isinstance(layer, nn.Linear):
#            if isinstance(layer, nn.Conv2d):
#                params = layer.weight.data[:2]
#                
#                name_layer = name
#                #if self.rank==0:
#                #print(f"first conv before backward: {model.conv1.weight.data[:10]}")
#                    #print(f"iteration:{self.n_iter}, name of layer: {name_layer}, params before backward: {params}")
#                break
                
        #features_sum = np.copy(self.extracted_feature_dict[batch_index])
        for node in self.selected_node:
            #print("neighbor_extracted_feature_dict:", neighbor_extracted_feature_dict)
            for features, outputs, selfdata, selflabels in zip(neighbor_extracted_feature_dict[node], neighbor_logits_dict[node], self.batchlist, self.targetlist):
                #features_sum += neighbor_extracted_feature_dict[node][batch_index]
            
                #print(f"batch index: {len(neighbor_extracted_feature_dict[node].keys())}")
                batch_feature = torch.from_numpy(features).cuda(non_blocking=True)
                batch_logits = torch.from_numpy(outputs).float().cuda(non_blocking=True)
                
            
                
               
                    
                output_batch, _ = model(batch_feature, neighbor_train = True)
                #print("Hiiiiiiiiiiiiii I set neighbor train to True")
                #print(f"output batch: {output_batch}")
                
                selflogits, _ = model(selfdata, neighbor_train = False)
                loss_kd = self.criterion_KL(output_batch, batch_logits).cuda(non_blocking=True) 
                
                #selffeature = torch.from_numpy(self.extracted_feature_dict[batch_index]).cuda(non_blocking=True)
                #selflabel = torch.from_numpy(labels).long().cuda(non_blocking=True)
                #selfoutput_batch, _ = model(selffeature, neighbor_train = True)
                loss_true = self.criterion(selflogits, selflabels)
                #loss = loss_kd + self.args.alpha * loss_true
                #loss_b = self.criterion(selflogits, selflabel)
                loss = 0.5*loss_kd + loss_true
                #loss = loss_kd
